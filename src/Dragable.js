...
import { Tree, Input, Select, Button, Checkbox } from "antd";
...
const SearchTree = () => {
    ...
    const helperGetItem = useCallback(
        (p = []) => {
    ...
    const helperDeleteItem = useCallback(
        (p = []) => {
    ...
    const helperSaveItem = useCallback(
        (p = [], params) => {
                    ...
                    if (title === "label") {
                        newTitle = (
                            <Folder
                                name={name}
                                type={type}
                                handleSelect={() => handleSelectTree(key)}
                            />
                        );
                    }
                    return {
    ...
    const filterOneByOne = useCallback((rawData) => {
    ...
    const filterTree = useCallback(
        (rawData) => {
            if (!searchValue) return rawData;
            if (checked) return filterOneByOne(rawData)
            const filterFunc = (data) => {
        ...
        [searchValue, checked, filterOneByOne]
    );
        ...
        <>
            ...
            >
                <Checkbox 
                    ...
                    onChange={(e) => setChecked(e.target.checked)}>One by one search</Checkbox>
                <Select
                    labelInValue
                    open={open}
                    style={{ width: 448 }}
                    onClick={(e) => {
                        ...
                        if (targetClass.includes(className) && id.length < 30) {
                            setOpen(!open);
                        }
                    }}
                    dropdownStyle={{
                ...
                <div className="g-action-parent">
                    ...
                    <Button disabled={!value} onClick={() => setSave(true)}>
                        Save
                    </Button>
                </div>
            </div>
            {save && (
                ...
                >
                    ...
                    <div>
                        ...
                        <br></br>
                        <span>
                            {!!value?.allowers?.length &&
                                `Visible to ${value.allowers.toString()}`}
                        </span>
                    </div>
                </div>
            )}
